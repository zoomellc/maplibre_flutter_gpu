// MapLibre simple-line vertex shader with independently data-driven paint.
// Native normalizes every DD line vertex to 120 bytes: float line geometry,
// paint ranges, and two float4 pattern endpoints.
#version 460 core
...
layout(location = 2) out vec2 v_gamma_dpr;
layout(location = 4) out vec4 v_color;
layout(location = 5) out vec2 v_blur_opacity;
layout(location = 6) out vec2 v_unused_floorwidth;
layout(location = 7) flat out vec4 v_unused_pattern_from;
layout(location = 8) flat out vec4 v_unused_pattern_to;
