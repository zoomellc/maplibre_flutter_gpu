// MapLibre gradient-line vertex shader for the normalized 120-byte DD line
// layout. The gradient texture supplies color; five scalar paints can vary.
#version 460 core
...
layout(location = 2) out float v_lineprogress;
layout(location = 3) out vec2 v_gamma_dpr;
layout(location = 5) out vec2 v_blur_opacity;
layout(location = 6) out vec4 v_unused_color;
layout(location = 7) out vec2 v_unused_floorwidth;
...
void main() {
    ...
    v_lineprogress =
        (floor(a_data.z / 4.0) + a_data.w * 64.0) * 2.0 / MAX_LINE_DISTANCE;
